...
- Реализуйте класс `com.home.webapp.storage.ArrayStorage`: хранение резюме на основе массива (методы `clear, get, save, delete, getAll, size`). Не используйте в решении коллекции, реализацию на их основе мы добавим позднее.
...
- Протестируйте вашу реализацию, запустив `com.home.webapp.MainTestArrayStorage.main()`: в IDEA слева на полях зеленая стрелка.
- Протестируйте вашу реализацию интерактивно с помощью `com.home.webapp.MainArray.main()`.
...
### Optional
- Модифицировать класс `com.home.webapp.storage.ArrayStorage`: хранить все резюме в начале storage (без дырок null), чтобы не перебирать каждый раз все 10000 элементов.
```
...
```
- Посмотреть на класс `Arrays`. Там есть полезные вещи, которые могут упростить код `com.home.webapp.storage.ArrayStorage`.
- Протестируйте реализацию, запустив com.home.webapp.MainArray.main(): в IDEA слева на полях зеленая стрелка.
